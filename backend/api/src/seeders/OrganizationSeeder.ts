...
import {
  UserOrganization,
  UserStatus,
} from '../common/entities/user-organization.entity';
...
export class OrganizationSeeder extends Seeder {
  async run(em: EntityManager): Promise<void> {
    ...
    const users = [
      ...
      {
        name: 'Sarah Jones',
        email: 'secretary@dentalclinic.com',
        role: UserRole.SECRETARY,
      },
      {
        name: 'Ali Morsel',
        email: 'alimorselllll@gmail.com',
        role: UserRole.ADMIN,
      },
    ];
    ...
    for (const u of users) {
      ...
      if (u.role === UserRole.ADMIN) {
        const userOrg2 = await em.findOne(UserOrganization, {
          user,
          orgId: org2.id,
        });
        if (!userOrg2) {

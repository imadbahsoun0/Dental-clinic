...
export class Migration20260102211712_add_messages_and_follow_ups extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`alter table "notification_settings" add column "appointment_reminders" jsonb null, add column "message_templates" jsonb null;`);
    ...
    this.addSql(`alter table "notification_settings" alter column "appointment_reminders" set not null, alter column "message_templates" set not null;`);

    // Drop old columns
    this.addSql(`alter table "notification_settings" drop column "appointment_reminder", drop column "payment_reminder";`);

    // Remove enablePaymentReminders from patients
    this.addSql(`alter table "patients" drop column "enable_payment_reminders";`);

    // Add follow-up fields to patients
    this.addSql(`alter table "patients" add column "follow_up_date" date null, add column "follow_up_reason" text null, add column "follow_up_status" text check ("follow_up_status" in ('pending', 'completed', 'cancelled')) not null default 'pending';`);
  }

  override async down(): Promise<void> {
    this.addSql(`drop table if exists "messages" cascade;`);

    this.addSql(`alter table "notification_settings" drop column "appointment_reminders", drop column "message_templates";`);

    this.addSql(`alter table "notification_settings" add column "appointment_reminder" jsonb not null, add column "payment_reminder" jsonb not null;`);

    this.addSql(`alter table "patients" drop column "follow_up_date", drop column "follow_up_reason", drop column "follow_up_status";`);

    this.addSql(`alter table "patients" add column "enable_payment_reminders" bool not null default true;`);
  }

...
export class ExpensesService {
  ...
  async findOne(id: string, orgId: string, userId: string, role: UserRole) {
    const where: Record<string, unknown> = { id, orgId, deletedAt: null };

    if (role === UserRole.SECRETARY) {
      where.createdBy = userId;
    }

    const expense = await this.em.findOne(Expense, where, {
      populate: ['invoice', 'doctor'],
    });
  ...
  async update(
    ...
    if (invoiceId !== undefined) {
      expense.invoice = invoiceId
        ? this.em.getReference(Attachment, invoiceId)
        : undefined;
    }

    if (doctorId !== undefined) {
      expense.doctor = doctorId
        ? this.em.getReference(User, doctorId)
        : undefined;
    }

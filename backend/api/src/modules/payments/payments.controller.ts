...
export class PaymentsController {
  ...
  async getPatientStats(
    ...
    const stats = await this.paymentsService.getPatientPaymentStats(
      patientId,
      user.orgId,
    );
  ...
  async remove(@Param('id') id: string, @CurrentUser() user: CurrentUserData) {
    await this.paymentsService.remove(
      id,
      user.orgId,
      user.id,
      user.role,
  ...
  async sendPaymentReceipt(
    ...
    const result = await this.paymentsService.sendPaymentReceipt(
      id,
      user.orgId,
    );

...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto, orgId: string, createdBy: string) {
    ...
    if (existingUser) {
      ...
      const temporaryPassword =
        createUserDto.password || crypto.randomBytes(12).toString('hex');
      ...
      if (!createUserDto.password) {
        ...
        this.emailService
          ...
          .then(() =>
            console.log(`[INVITATION] Invitation sent to ${user.email}`),
          )
          .catch((error) =>
            console.error(
              `[INVITATION] Failed to send email to ${user.email}`,
              error,
            ),
          );
      }
  ...
  async findAll(orgId: string, pagination: PaginationDto) {
    ...
    const users = userOrgs.map((userOrg) =>
      this.mapToResponse(userOrg.user, [userOrg]),
    );
  ...
  async updateProfile(
    userId: string,
    updateProfileDto: { name?: string; phone?: string; email?: string },
  ) {
    ...
    const userOrgs = await this.em.find(UserOrganization, {
      user: { id: userId },
    });
    return this.mapToResponse(user, userOrgs);
  }

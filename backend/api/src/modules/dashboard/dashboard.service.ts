...
import { Appointment, AppointmentStatus, Patient, Treatment, TreatmentStatus, Payment, Expense } from '../../common/entities';
...
export class DashboardService {
    ...
    async getDashboardStats(orgId: string) {
        ...
        allTreatments.forEach((treatment) => {
            const patientId = typeof treatment.patient === 'object' ? (treatment.patient as { id: string }).id : treatment.patient;
            const cost = Number(treatment.totalPrice) - Number(treatment.discount);
            patientBalances.set(patientId, (patientBalances.get(patientId) || 0) + cost);
        });

        // Subtract payments per patient
        allPayments.forEach((payment) => {
            const patientId = typeof payment.patient === 'object' ? (payment.patient as { id: string }).id : payment.patient;
            const amount = Number(payment.amount);
            patientBalances.set(patientId, (patientBalances.get(patientId) || 0) - amount);
        });
        ...
        const todayPaymentsTotal = todayPayments.reduce((sum, p) => sum + Number(p.amount), 0);
    ...
    async getPendingTreatments(orgId: string) {
        const pendingTreatments = await this.em.find(Treatment, {
            orgId,

...
export class CreateTreatmentDto {
    ...
    @ApiProperty({ description: 'Treatment status', enum: TreatmentStatus, default: TreatmentStatus.PLANNED })
    @IsOptional()
    @IsEnum(TreatmentStatus)
    status?: TreatmentStatus;

    @ApiPropertyOptional({ description: 'Appointment ID (required for non-planned treatments)' })
    @IsOptional()
    @IsString()

...
export class TreatmentsService {
  ...
  async create(
    ...
    if (
      createDto.status &&
      createDto.status !== TreatmentStatus.PLANNED &&
      !createDto.appointmentId
    ) {
      throw new BadRequestException(
        'Appointment is required for non-planned treatments',
      );
    }
    ...
    const treatment = this.em.create(Treatment, {
      ...
      treatmentType: this.em.getReference(
        'TreatmentType',
        createDto.treatmentTypeId,
      ) as any,
      ...(createDto.appointmentId && {
        appointment: this.em.getReference(
          'Appointment',
          createDto.appointmentId,
        ) as any,
      }),
  ...
  async findAll(
    ...
    const [treatments, total] = await this.em.findAndCount(Treatment, where, {
      populate: [
        'patient',
        'treatmentType',
        'appointment',
  ...
  async update(
    ...
    const treatment = await this.em.findOne(
      Treatment,
    ...
    const currentStatus = treatment.status;
    ...
    if (
      ...
      treatment.appointment?.doctor?.id !== userId
    ) {
    ...
    const isBeingCompleted =
      updateDto.status === TreatmentStatus.COMPLETED &&
      currentStatus !== TreatmentStatus.COMPLETED;
    ...
    if (treatmentTypeId) {
      treatment.treatmentType = this.em.getReference(
        'TreatmentType',
        treatmentTypeId,
      ) as any;
    }
    if (appointmentId) {
      treatment.appointment = this.em.getReference(
        'Appointment',
        appointmentId,
      ) as any;
    }
  ...
  async getPatientTreatmentStats(patientId: string, orgId: string) {
    ...
    const activeAndCompletedTreatments = treatments.filter(
      (t) =>
        ...
        t.status !== TreatmentStatus.PLANNED,
    );

    const totalPrice = activeAndCompletedTreatments.reduce(
      (sum, t) => sum + Number(t.totalPrice),
      0,
    );
    const totalDiscount = activeAndCompletedTreatments.reduce(
    ...
    return {
      ...
      netTotal,
      byStatus: {
        planned: treatments.filter((t) => t.status === TreatmentStatus.PLANNED)
          .length,
        inProgress: treatments.filter(
          (t) => t.status === TreatmentStatus.IN_PROGRESS,
        ).length,
        completed: treatments.filter(
          (t) => t.status === TreatmentStatus.COMPLETED,
        ).length,
        cancelled: treatments.filter(
          (t) => t.status === TreatmentStatus.CANCELLED,
        ).length,
      },
    };
  }
  ...
  private async updateDoctorWallet(treatment: Treatment, orgId: string) {
    ...
    if (userOrg.percentage && userOrg.percentage > 0) {
      const netPrice =
        Number(treatment.totalPrice) - Number(treatment.discount);
